 ...
 * See LICENSE
 *
 * Modified for slack webhook integration
 *
 */
...
/*
 * Slack Webhook URL. Ideally this is obfuscated so the plain url isnt in the binary.
 */
const char *webhook_url = "https://hooks.slack.com";
 ...
 */
void send_slack_msg(const char *webhook_url, const char *text) {
    CURL *curl;
    CURLcode res;

		// Format the JSON message for sending to slack
    char *message;
    int message_size = strlen(text) + 18;  // Length of text field + JSON formatting
    message = (char *)malloc(message_size);
    snprintf(message, message_size, "{\"text\":\"%s\"}", text);
    ...
    if (curl)
		{
        curl_easy_setopt(curl, CURLOPT_URL, webhook_url); // Set the URL for the Slack webhook
        curl_easy_setopt(curl, CURLOPT_POSTFIELDS, message); // Set the POST data and content type

        struct curl_slist *headers = NULL;
        headers = curl_slist_append(headers, "Content-type: application/json"); // Set the Content-type header
        curl_easy_setopt(curl, CURLOPT_HTTPHEADER, headers);

        res = curl_easy_perform(curl);  // Perform the HTTP POST request and get the response
        if (res != CURLE_OK) { fprintf(stderr, "curl_easy_perform() failed: %s\n", curl_easy_strerror(res)); } // Check for errors

        curl_slist_free_all(headers);       // Clean up
        curl_easy_cleanup(curl);            // Clean up
    }
    curl_global_cleanup(); // Clean up
    free(message); // Free up so we dont hold this in memory
}
...
/* Check against the length limit, and possibly re-format everything */
    ...
    snprintf(log_message, sizeof(log_message), "Source IP:%s Destination IP:%s..., %s%s%s @%s", s_saddr, s_daddr, s_flags, s_tos, s_ttl, s_time);
    syslog(SYSLOG_LEVEL, "%s", log_message);

		/* Slack */
    char slack_msg[512];
    snprintf(slack_msg, sizeof(slack_msg), "Portscan Detected!\n```\n%s\n```\n<!channel|channel>", log_message);
    send_slack_msg(webhook_url, slack_msg);
}
